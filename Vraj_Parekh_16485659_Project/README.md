...
Upload the four sequential notebooks from the /notebooks/ folder to Google Colab.

Download the original HIGGS dataset from the UCI Machine Learning Repository, mount your Google Drive, and ensure the CSV is placed at this exact path: /content/drive/MyDrive/Bigdata/HIGGS/HIGGS.csv.

Run the notebooks sequentially (1 through 4). The pipeline handles data ingestion, feature engineering, model training, and evaluation.

Option 2: Automated Execution (Terminal/Local Spark)

Ensure Apache Spark is installed and configured on your local machine or cluster.

Run the command python scripts/run_pipeline.py to execute the entire pipeline end-to-end automatically.
